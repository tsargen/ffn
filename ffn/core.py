...
class PerformanceStats(object):
    ...
    def __init__(self, prices, rf=0., returns_type=None):
    ...
    def _calculate(self, obj):
        ...
        self.rolling_1m_drawdown = np.nan  # NEW x
        self.rolling_3m_drawdown = np.nan  # NEW x
        self.max_drawdown = np.nan
        self.max_1m_drawdown = np.nan  # NEW #
        self.max_3m_drawdown = np.nan  # NEW #
        ...
        self.spreadshock = np.nan  # NEW
        self.max_spreadshock = np.nan  # NEW
        self.spreadshock_details = np.nan  # NEW
        self.avg_spreadshock = np.nan  # NEW
        self.avg_spreadshock_days = np.nan  # NEW
        ...
        self.start = obj.dropna(how='all').index[0]
        self.end = obj.dropna(how='all').index[-1]
        ...
        self.changes = dp.diff()
        self.log_returns = dp.to_log_returns()

        r = self.returns
        if self.returns_type == 'Change':
            r = self.changes
        elif self.returns_type == 'Log':
            r = self.log_returns
        elif self.returns_type is None:
            r = self.returns
        else:
            raise NotImplementedError('Unsupported return type {}'.format(self.returns_type))
        ...
        ## Drop N/A
        self.pos_day_perc = len(r[r > 0]) / float(len(r.dropna(how='all')))

        self.total_return = obj[-1] / obj[0] - 1

        if self.returns_type == 'Change':
            self.total_return = obj[1] - obj[0]
        ...
        self.rolling_1m_drawdown = calc_rolling_drawdown(dp, 22)
        self.rolling_3m_drawdown = calc_rolling_drawdown(dp, 66)
        self.max_1m_drawdown = self.rolling_1m_drawdown.min()
        self.max_3m_drawdown = self.rolling_3m_drawdown.min()
        self.calmar = np.divide(self.cagr, np.abs(self.max_drawdown))

        ## spread change calculations
        self.spreadshock = dp.to_spreadshock_series(self.returns_type)
        self.max_spreadshock = self.spreadshock.max()
        self.spreadshock_details = spreadshock_details(self.spreadshock)
        if self.spreadshock_details is not None:
            self.avg_spreadshock = self.spreadshock_details['spreadshock'].mean()
            self.avg_spreadshock_days = self.spreadshock_details['Length'].mean()
        self.rolling_1m_spreadshock = calc_rolling_spreadshock(dp, 22, returns_type=self.returns_type)
        self.rolling_3m_spreadshock = calc_rolling_spreadshock(dp, 66, returns_type=self.returns_type)
        self.max_1m_spreadshock = self.rolling_1m_spreadshock.max()
        self.max_3m_spreadshock = self.rolling_3m_spreadshock.max()
        ...
        if self.returns_type == 'Change':
            self.monthly_returns = self.monthly_prices.diff()
            mr = self.monthly_returns
        elif self.returns_type == None:
            self.monthly_returns = self.monthly_prices.to_returns()
            mr = self.monthly_returns
        else:
            raise NotImplementedError('Unsupported return type {}'.format(self.returns_type))
        ...
        self.pos_month_perc = len(mr[mr > 0]) / len(mr.dropna(how='all'))
        self.avg_up_month = mr[mr > 0].mean()
        self.avg_down_month = mr[mr <= 0].mean()

        # descriptive stats for spreads
        des_stats = self.prices.describe([0.1,0.9])
        with np.errstate(invalid='ignore'):
            self.last_perc = scipy.stats.percentileofscore(self.prices, self.prices[-1])
        self.count = des_stats['count']
        self.mean = des_stats['mean']
        self.std = des_stats['std']
        self.min = des_stats['min']
        self.r10th= des_stats['10%']
        self.r50th= des_stats['50%']
        self.r90th= des_stats['90%']
        self.max = des_stats['max']
        ...
        try:
            ...
            if self.returns_type == 'Change':
                self.return_table[fidx.year][fidx.month] = float(mp[0]) - dp[0]
        ...
        for idx in self.return_table:
            ...
            if self.returns_type == None:
                self.return_table[idx][13] = np.prod(arr + 1) - 1
            else:
                self.return_table[idx][13] = np.sum(arr)
        ...
        # -1 here to account for first return that will be nan
        self.win_year_perc = len(yr[yr > 0]) / len(yr.dropna())
    ...
    def display_monthly_returns(self):
        ...
        for k in self.return_table.index:
            ...
            if self.returns_type == None:
                ...
                data.append([k] + [fmtn(x) for x in r])
...
class SpreadPerformanceStats(PerformanceStats):
    """
    PerformanceStats is a convenience class used for the performance
    evaluation of a spread series. It contains various helper functions
    to help with plotting and contains a large amount of descriptive
    statistics.  Output has been adjusted for spread calculations.

    Args:
        * prices (Series): A spread series.
        * rf (float, Series): Risk-free rate used in various calculation. Should be
            expressed as a yearly (annualized) return if it is a float. Otherwise
            rf should be a price series.
    ...
    def _stats(self):
        ...
        stats = [('start', 'Start', 'dt'),
                 ('end', 'End', 'dt'),
                 (None, None, None),
                 ('total_return', 'Total Change', 'n'),
                 ('max_spreadshock', 'Max Spread Shock', 'n'),
                 ('max_1m_spreadshock', 'Rolling 1m Max Spread Shock', 'p'),
                 ('max_3m_spreadshock', 'Rolling 3m Max Spread Shock', 'p'),
                 (None, None, None),
                 ('mtd', 'MTD', 'n'),
                 ('three_month', '3m', 'n'),
                 ('six_month', '6m', 'n'),
                 ...
                 ('one_year', '1Y', 'n'),
                 (None, None, None),
                 ('daily_mean', 'Daily Mean (ann.)', 'n'),
                 ('daily_vol', 'Daily Vol (ann.)', 'n'),
                 ('best_day', 'Biggest Widening Day', 'n'),
                 ('worst_day', 'Biggest Tightening Day', 'n'),
                 ('pos_day_perc', 'Widen Daily %', 'p'),
                 (None, None, None),
                 ('monthly_mean', 'Monthly Mean (ann.)', 'p'),
                 ('monthly_vol', 'Monthly Vol (ann.)', 'p'),
                 ('best_month', 'Biggest Widening Month', 'n'),
                 ('worst_month', 'Biggest Tightening Month', 'n'),
                 ('pos_month_perc', 'Widen % - Monthly', 'p'),
                 (None, None, None),
                 ('yearly_mean', 'Yearly Mean', 'n'),
                 ('yearly_vol', 'Yearly Vol', 'n'),
                 ('best_year', 'Biggest Widening Year', 'n'),
                 ('worst_year', 'Biggest Tightening Year', 'n'),
                 (None, None, None),
                 ('avg_spreadshock', 'Avg. Spread Shock', 'p'),
                 ('avg_spreadshock_days', 'Avg. Spread Shock Days', 'n'),
                 ('avg_up_month', 'Avg. Widen Month', 'n'),
                 ('avg_down_month', 'Avg. Tighten Month', 'p'),
                 ('win_year_perc', 'Widen Year %', 'p'),
                 ('twelve_month_win_perc', 'Widen 12m %', 'p'),
                 (None, None, None),
                 ('count', 'N Obs', 'n'),
                 ('mean', 'Average', 'n'),
                 ('std', 'StDev', 'n'),
    ...
    def display(self):
        ...
        if self.returns_type == 'Change':
            print('Numbers reflect Spread changes over the period')
        print('Summary:')
        if self.returns_type == 'Change':
            data = [[fmtn(self.total_return), fmtn(self.max_spreadshock),
                     fmtn(self.max_1m_spreadshock), fmtn(self.max_3m_spreadshock)]]
        else:
            data = [[fmtp(self.total_return), fmtp(self.max_spreadshock),
                     fmtp(self.max_1m_spreadshock), fmtp(self.max_3m_spreadshock)]]
        print(tabulate(data, headers=['Total Spread Change',
                                      'Max Spread Shock',
                                      'Max 1m Spread Shock',
                                      'Max 3m Spread Shock']))
        ...
        if self.returns_type == 'Change':
            data = [[fmtn(self.mtd), fmtn(self.three_month), fmtn(self.six_month),
                     fmtn(self.ytd), fmtn(self.one_year)]]
        else:
            data = [[fmtp(self.mtd), fmtp(self.three_month), fmtp(self.six_month),
                     fmtp(self.ytd), fmtp(self.one_year)]]

        print(tabulate(data, headers=['mtd', '3m', '6m', 'ytd', '1y']))

        print('\nPeriodic:')
        if self.returns_type == 'Change':
            data = [
                ['mean', fmtn(self.daily_mean), fmtn(self.monthly_mean),
                 fmtn(self.yearly_mean)],
                ['vol', fmtn(self.daily_vol), fmtn(self.monthly_vol),
                 fmtn(self.yearly_vol)],
                ['worst', fmtn(self.best_day), fmtn(self.best_month),
                 fmtn(self.best_year)],
                ['best', fmtn(self.worst_day), fmtn(self.worst_month),
                 fmtn(self.worst_year)]]
        else:
            data = [
                ...
                ['worst', fmtp(self.best_day), fmtp(self.best_month),
                 fmtp(self.best_year)],
                ['best', fmtp(self.worst_day), fmtp(self.worst_month),
                 fmtp(self.worst_year)]]
        print(tabulate(data, headers=['daily', 'monthly', 'yearly']))

        print('\nSpread Shocks:')
        data = [
            [fmtn(self.max_spreadshock), fmtn(self.avg_spreadshock),
             fmtn(self.avg_spreadshock_days)]]
        print(tabulate(data, headers=['max', 'avg', '# days']))

        print('\nSpread Ranges:')
        data = [['n. obs', fmtn(self.count)],
                ['avg spread', fmtn(self.mean)],
                ['stdev', fmtn(self.std)],
                ['min', fmtn(self.min)],
                ['10%', fmtn(self.r10th)],
                ['50%', fmtn(self.r50th)],
                ['90%', fmtn(self.r90th)],
                ['max', fmtn(self.max)],
                ['last perc', fmtn(self.last_perc)]]
        print(tabulate(data))
            
        print('\nMisc:')
        if self.returns_type == 'Change':
            data = [['avg. widen month', fmtn(self.avg_up_month)],
                    ['avg. tighten month', fmtn(self.avg_down_month)],
                    ['widen day %', fmtp(self.pos_day_perc)],
                    ['widen month %', fmtp(self.pos_month_perc)],
                    ['widen year %', fmtp(self.win_year_perc)],
                    ['12m widen %', fmtp(self.twelve_month_win_perc)]]
        else:
            data = [['avg. widen month', fmtp(self.avg_up_month)],
                    ['avg. tighten month', fmtp(self.avg_down_month)],
                    ['widen day %', fmtp(self.pos_day_perc)],
                    ['widen month %', fmtp(self.pos_month_perc)],
                    ['widen year %', fmtp(self.win_year_perc)],
                    ['12m widen %', fmtp(self.twelve_month_win_perc)]]
...
class PricePerformanceStats(PerformanceStats):
    """
    PerformanceStats is a convenience class used for the performance
    evaluation of a price series. It contains various helper functions
    to help with plotting and contains a large amount of descriptive
    statistics.  Data output has been modified to suit spread series
    ...
    def _stats(self):
        ...
        stats = [('start', 'Start', 'dt'),
                 ...
                 ('max_1m_drawdown', 'Rolling 1m Max Drawdown', 'p'),
                 ('max_3m_drawdown', 'Rolling 3m Max Drawdown', 'p'),
    ...
    def display(self):
        ...
        data = [[fmtp(self.total_return), fmtn(self.daily_sharpe),
                 ...
                 fmtp(self.max_1m_drawdown), fmtp(self.max_3m_drawdown)]]
        ...
        data = [['avg. up month', fmtp(self.avg_up_month)],
                ...
                ['up day %', fmtp(self.pos_day_perc)],
                ['up month %', fmtp(self.pos_month_perc)],
...
class GroupStats(dict):
    ...
    def __init__(self, *prices, price_type='Price', returns_type=None):
        names = []
        for p in prices:
        ...
        self._returns_type = returns_type
    ...
    def _calculate(self, data):
        ...
        for c in data.columns:
            prc = data[c]
            if self._price_type == 'Price':
                self[c] = PricePerformanceStats(prc)
            elif self._price_type == 'Spread':
                self[c] = SpreadPerformanceStats(prc, returns_type=self._returns_type)
            else:
                raise(NotImplementedError,
                      'Price type {} not implemented. Price or Spread supported'.format(self._price_type))
    ...
    def _stats(self):
        ...
        if self._price_type == 'Price':
        ...
        elif (self._price_type == 'Spread' and self._returns_type == None):
            stats = [('start', 'Start', 'dt'),
                     ('end', 'End', 'dt'),
                     (None, None, None),
                     ('total_return', 'Total Change', 'p'),
                     ('max_spreadshock', 'Max Spread Shock', 'p'),
                     ('max_1m_spreadshock', 'Rolling 1m Max Spread Shock', 'p'),
                     ('max_3m_spreadshock', 'Rolling 3m Max Spread Shock', 'p'),
                     (None, None, None),
                     ('mtd', 'MTD', 'p'),
                     ...
                     ('daily_mean', 'Daily Mean (ann.)', 'p'),
                     ('daily_vol', 'Daily Vol (ann.)', 'p'),
                     ('best_day', 'Biggest Widening Day', 'p'),
                     ('worst_day', 'Biggest Tightening Day', 'p'),
                     ('pos_day_perc', 'Widen Daily %', 'p'),
                     (None, None, None),
                     ('monthly_mean', 'Monthly Mean (ann.)', 'p'),
                     ('monthly_vol', 'Monthly Vol (ann.)', 'p'),
                     ('best_month', 'Biggest Widening Month', 'p'),
                     ('worst_month', 'Biggest Tightening Month', 'p'),
                     ('pos_month_perc', 'Widen % - Monthly', 'p'),
                     (None, None, None),
                     ('yearly_mean', 'Yearly Mean', 'p'),
                     ('yearly_vol', 'Yearly Vol', 'p'),
                     ('best_year', 'Biggest Widening Year', 'p'),
                     ('worst_year', 'Biggest Tightening Year', 'p'),
                     (None, None, None),
                     ('avg_spreadshock', 'Avg. Spread Shock', 'p'),
                     ('avg_spreadshock_days', 'Avg. Spread Shock Days', 'n'),
                     ('avg_up_month', 'Avg. Widen Month', 'p'),
                     ('avg_down_month', 'Avg. Tighten Month', 'p'),
                     ('win_year_perc', 'Widen Year %', 'p'),
                     ('twelve_month_win_perc', 'Widen 12m %', 'p'),
                     (None, None, None),
        ...
        elif (self._price_type == 'Spread' and self._returns_type == 'Change'):
            stats = [('start', 'Start', 'dt'),
                     ('end', 'End', 'dt'),
                     (None, None, None),
                     ('total_return', 'Total Change', 'n'),
                     ('max_spreadshock', 'Max Spread Shock', 'n'),
                     ('max_1m_spreadshock', 'Rolling 1m Max Spread Shock', 'n'),
                     ('max_3m_spreadshock', 'Rolling 3m Max Spread Shock', 'n'),
                     (None, None, None),
                     ('mtd', 'MTD', 'n'),
                     ('three_month', '3m', 'n'),
                     ('six_month', '6m', 'n'),
                     ...
                     ('pos_day_perc', 'Widen Daily %', 'p'),
                     (None, None, None),
                     ('monthly_mean', 'Monthly Mean (ann.)', 'n'),
                     ('monthly_vol', 'Monthly Vol (ann.)', 'n'),
                     ('best_month', 'Biggest Widening Month', 'n'),
                     ('worst_month', 'Biggest Tightening Month', 'n'),
                     ('pos_month_perc', 'Widen % - Monthly', 'p'),
                     (None, None, None),
                     ...
                     ('worst_year', 'Biggest Tightening Year', 'n'),
                     (None, None, None),
                     ('avg_spreadshock', 'Avg. Spread Shock', 'n'),
                     ('avg_spreadshock_days', 'Avg. Spread Shock Days', 'n'),
                     ('avg_up_month', 'Avg. Widen Month', 'n'),
                     ('avg_down_month', 'Avg. Tighten Month', 'n'),
                     ('win_year_perc', 'Widen Year %', 'p'),
                     ('twelve_month_win_perc', 'Widen 12m %', 'p'),
                     (None, None, None),
                     ('count', 'N Obs', 'n'),
            ...
            raise(NotImplementedError,
                  """Price type {} not implemented.  
                  Price or Spread supported""".format(self._price_type))
        return stats
...
def calc_perf_stats(prices, rf=0., price_type='Prices', returns_type=None):
    """
    ...
    """
    return PerformanceStats(prices, rf, price_type, returns_type)


def calc_stats(prices, rf=0., price_type='Prices', returns_type=None):
    """
...
def extend_pandas():
    ...
    PandasObject.to_spreadshock_series = to_spreadshock_series


def to_spreadshock_series(spreads, returns_type=None):
    """
    Calculates the spredshock series.
    This returns a series representing a spread widening.
    When the spread is at all time lows, the shock
    is 0. However, when spreads are above the low water marks,
    the spreadshock series = current / lwm - 1
    The max widening can be obtained by simply calling .max()
    on the result (since the shock series is positive)
    Method ignores all gaps of NaN's in the price series.
    Args:
        * spreads (Series or DataFrame): Series of spreads.
    """
    # make a copy so that we don't modify original data
    spreadshock = spreads.copy()
    # Fill NaN's with previous values
    spreadshock = spreadshock.fillna(method='ffill')
    # Ignore problems with NaN's in the beginning
    spreadshock[np.isnan(spreadshock)] = -np.Inf
    # Rolling min
    roll_min = np.minimum.accumulate(spreadshock)
    if returns_type == None:
        spreadshock = spreadshock / roll_min - 1.
    elif (returns_type == 'Change' or returns_type == 'Log'):
        spreadshock = spreadshock - roll_min
    else:
        raise NotImplementedError('Unsupported return type {}'.format(returns_type))
    return spreadshock


## SPREAD CODE AND ROLLING APPLY FUNCTIONS
def spreadshock_details(spreadshock, index_type=pd.DatetimeIndex):
    """
    Returns a data frame with start, end, days (duration) and
    spreadshock for each spreadshock in a spreadshock series.
    .. note::
        days are actual calendar days, not trading days
    Args:
        * spreadshock (pandas.Series): A spreadshock Series
            (can be obtained w/ spreadshock(prices).
    Returns:
        * pandas.DataFrame -- A data frame with the following
            columns: start, end, days, spreadshock.
    """
    is_zero = spreadshock == 0
    # find start dates (first day where dd is non-zero after a zero)
    start = ~is_zero & is_zero.shift(1)
    start = list(start[start == True].index)  # NOQA
    # find end dates (first day where dd is 0 after non-zero)
    ...
    if len(start) is 0:
        return None
    # spreadshock has no end (end period in dd)
    if len(end) is 0:
        end.append(spreadshock.index[-1])
    # if the first spreadshock start is larger than the first spreadshock end it
    # means the spreadshock series begins in a spreadshock and therefore we must add
    # the first index to the start series
    if start[0] > end[0]:
        start.insert(0, spreadshock.index[0])
    # if the last start is greater than the end then we must add the last index
    # to the end series since the spreadshock series must finish with a spreadshock
    if start[-1] > end[-1]:
        end.append(spreadshock.index[-1])
    result = pd.DataFrame(
        columns=('Start', 'End', 'Length', 'spreadshock'),
        index=range(0, len(start))
    )
    for i in range(0, len(start)):
        dd = spreadshock[start[i]:end[i]].max()
        if index_type is pd.DatetimeIndex:
            result.iloc[i] = (start[i], end[i], (end[i] - start[i]).days, dd)
        else:
            result.iloc[i] = (start[i], end[i], (end[i] - start[i]), dd)
...
def calc_max_spreadshock(spreads, returns_type=None):
    """
    Calculates the max spreadshock of a price series. If you want the
    actual spreadshock series, please use to_spreadshock_series.
    """
    if returns_type is None:
        max_spreadshock = (spreads / spreads.expanding(min_periods=1).min()).max() - 1
    elif (returns_type == 'Change' or returns_type == 'Log'):
        max_spreadshock = (spreads - spreads.expanding(min_periods=1).min()).max()
    else:
        raise NotImplementedError('Unsupported return type {}'.format(returns_type))
    return max_spreadshock
...
def calc_rolling_spreadshock(spreads, window=60, returns_type=None, **kwargs):
    """
    Calculates a rolling spreadshock of a spread series.
    """
    return (spreads.rolling(window, **kwargs).apply(lambda x: calc_max_spreadshock(x, returns_type=returns_type)))


def calc_rolling_drawdown(prices, window=60, **kwargs):
    """
    Calculates a rolling drawdown of a price series
    """
    return (prices.rolling(window, **kwargs).apply(calc_max_drawdown))
